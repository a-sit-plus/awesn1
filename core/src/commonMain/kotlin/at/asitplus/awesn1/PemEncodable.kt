...
private constructor(private val decoders: Map<String, PemDecoder<T>>) : Asn1Decodable<A, T> {
    ...
    constructor(vararg decoders: Pair<String, ((ByteArray) -> T)?>) : this(decoders.associate {
        it.first to PemDecoder(
            it.second
        )
    })
    ...
    fun decodeFromPem(src: String): T = catchingUnwrapped {
        src.lineSequence()
            .map(String::trim)
            ...
            .let {
                ...
                while (it.hasNext()) {
                    ...
                    if (line.startsWith(FENCE_PREFIX_END) && line.endsWith(FENCE_SUFFIX)) {
                        ...
                        return@let Base64.Mime.decode(b64data.toString()).let { data ->
                            when (decoder) {
...
fun <A : Asn1Element, T : PemEncodable<A>> PemDecodable<A, T>.decodeFromPemOrNull(src: String): T? =
    catchingUnwrapped { decodeFromPem(src) }.getOrNull()
...
@OptIn(ExperimentalEncodingApi::class)
@Throws(IllegalArgumentException::class, Asn1Exception::class)
fun PemEncodable<*>.encodeToPem(): String = catchingUnwrapped {
