package at.asitplus.awesn1.serialization.api.backed
...
val MixedPolyTestsBacked by matrixSuite(matrixConfig { execution= ExecutionMode.Sequential }) {
    val a = Choice.A
    val b = Choice.B
    ...
    ).asData(name = "choice") test { (obj, hex) ->
    ...
    ).asData(name = "nested") test { (obj, hex) ->
        val encoded =
            Buffer().apply { der.encodeToSink(obj, this) }.readByteArray()
        println(encoded.toHexString())
        encoded.toHexString() shouldBe hex
        der.decodeFromSource<Choice.Nested>(Buffer().apply { write(encoded) }) shouldBe obj
    }
}
