...
val SerializationTestCoverageGaps by matrixSuite(
    ...
    "Asn1Tag INFER defaults class to context-specific while inferring constructed" {
        ...
        DER.decodeFromSource<InferTagOnPrimitive>(Buffer().apply { write(
            Buffer().apply { DER.encodeToSink(value, this) }.readByteArray()
        ) }) shouldBe value
    }
    ...
    "Asn1Tag INFER rejects foreign wire tag classes" {
        listOf("3003450107", "3003c50107", "3003050107").forEach { encoded ->
            shouldThrow<SerializationException> {
                DER.decodeFromSource<InferTagOnPrimitive>(Buffer().apply { write(encoded.hexToByteArray()) })
    ...
    "Asn1Tag constructed=INFER keeps constructed=true for class-level structures" {
        ...
        val element = Asn1Element.parse(Buffer().apply { DER.encodeToSink(value, this) }.readByteArray())

        element.tag.tagClass shouldBe TagClass.CONTEXT_SPECIFIC
        element.tag.isConstructed shouldBe true
        element.tag.tagValue shouldBe 3UL
        DER.decodeFromSource<InferConstructedOnClass>(Buffer().apply { write(Buffer().apply { DER.encodeToSink(value, this) }.readByteArray()) }) shouldBe value
    }

    "Asn1Explicit requires an explicit context-specific constructed tag" {
        shouldThrow<SerializationException> {
            Buffer().apply { DER.encodeToSink(ExplicitNoTag(ExplicitlyTagged(5)), this) }.readByteArray()
        }
        shouldThrow<SerializationException> {
            Buffer().apply { DER.encodeToSink(ExplicitlyTagged(5), this) }.readByteArray()
        }
        shouldThrow<SerializationException> {
            Buffer().apply { DER.encodeToSink(ExplicitWrongConstructed(ExplicitlyTagged(5)), this) }.readByteArray()
        }
    }

    "Asn1BitString on non-ByteArray targets is currently ignored (no shape change)" {
        val value = InvalidBitStringTarget(1)
        val encoded = Buffer().apply { DER.encodeToSink(value, this) }.readByteArray()
        encoded.toHexString() shouldBe "3003020101"
        DER.decodeFromSource<InvalidBitStringTarget>(Buffer().apply { write(encoded) }) shouldBe value
    }
    ...
    "Open polymorphism by OID supports custom OID selector wiring" {
        ...
        selectorDer.decodeFromSource<CustomSelectorBase>(Buffer().apply { write(Buffer().apply { selectorDer.encodeToSink(valueA, this) }.readByteArray()) }) shouldBe valueA
        selectorDer.decodeFromSource<CustomSelectorBase>(Buffer().apply { write(Buffer().apply { selectorDer.encodeToSink(valueB, this) }.readByteArray()) }) shouldBe valueB
    }
    ...
    "encodeDefaults=false and explicitNulls=true compose as expected" {
        ...
        val encoded = Buffer().apply { der.encodeToSink(withRequiredNull, this) }.readByteArray()
        ...
        der.decodeFromSource<CombinedFlagsModel>(Buffer().apply { write(encoded) }) shouldBe withRequiredNull
    }
}
...
private data class InvalidBitStringTarget(
    @at.asitplus.awesn1.serialization.Asn1BitString val value: Int,
)
