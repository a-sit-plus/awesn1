...
val SerializationTestCustomSerializerLeadingTags by testSuite(
    ...
    "Custom serializer with unknown leadingTags is rejected in undecidable optional middle position" {
        ...
        shouldThrow<SerializationException> {
            Buffer().apply { derNoDefaults.encodeToSink(value, this) }.readByteArray()
        }
        shouldThrow<SerializationException> {
            derNoDefaults.decodeFromSource<UnknownLeadingTagsOptionalMiddle>(Buffer().apply { write("3006020101020102".hexToByteArray()) })
        }
    }
    ...
    "Custom serializer with exact leadingTags enables implicit tag override inference" {
        ...
        derNoDefaults.decodeFromSource<KnownLeadingTagsOptionalMiddle>(Buffer().apply { write(
            Buffer().apply { derNoDefaults.encodeToSink(omitted, this) }.readByteArray()
        ) }) shouldBe omitted
        ...
        val encodedDer = Buffer().apply { derNoDefaults.encodeToSink(present, this) }.readByteArray()
        val reparsed = derNoDefaults.decodeFromSource<Asn1Element>(Buffer().apply { write(encodedDer) }).asStructure()
        ...
        derNoDefaults.decodeFromSource<KnownLeadingTagsOptionalMiddle>(Buffer().apply { write(encodedDer) }) shouldBe present
    }
}
