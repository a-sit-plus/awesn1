...
val SerializationTestOpenPolymorphismByOid by testSuite(
    ...
    "Open-polymorphic OID dispatch round-trips with registered subtypes" {
        ...
        der.decodeFromSource<OpenByOid>(Buffer().apply { write(
    ...
    "Additional OID subtype can be enabled by extending the DER serializers module" {
        ...
        extendedDer.decodeFromSource<OpenByOid>(Buffer().apply { write(encoded) }) shouldBe boolValue

        shouldThrow<SerializationException> {
            strictDer.decodeFromSource<OpenByOid>(Buffer().apply { write(encoded) })
        }.message.shouldContain("for OID")
    }

    "Missing OID discriminator fails decode" {
        val der = derWithOpenByOid(includeBool = false)
        val encodedNoOid = Buffer().apply { der.encodeToSink(NoOidEnvelope.serializer(), NoOidEnvelope(value = 1), this) }.readByteArray()
        shouldThrow<SerializationException> {
            der.decodeFromSource<OpenByOid>(Buffer().apply { write(encodedNoOid) })
