package at.asitplus.awesn1.serialization.api
import at.asitplus.awesn1.io.decodeFromSource
import at.asitplus.awesn1.io.encodeToSink
import kotlinx.io.Buffer
import kotlinx.io.readByteArray
import at.asitplus.awesn1.serialization.*


import at.asitplus.testballoon.invoke
import de.infix.testBalloon.framework.core.TestConfig
import de.infix.testBalloon.framework.core.TestSession.Companion.DefaultConfiguration
import de.infix.testBalloon.framework.core.invocation
import de.infix.testBalloon.framework.core.testSuite
import io.kotest.assertions.throwables.shouldThrow
import io.kotest.matchers.shouldBe
import kotlinx.serialization.Serializable
import kotlinx.serialization.SerializationException
import kotlinx.serialization.decodeFromByteArray
import kotlinx.serialization.encodeToByteArray
import kotlinx.serialization.modules.SerializersModule

@OptIn(ExperimentalStdlibApi::class)
val SerializationTestPolymorphismAndChoice by testSuite(
    testConfig = DefaultConfiguration.invocation(TestConfig.Invocation.Sequential)
) {
    "Number of elements" {
        val withThreeOtherNull = WithThreeNullable("first", null, "3")

        Buffer().apply { DER.encodeToSink(withThreeOtherNull, this) }.readByteArray().also { bytes ->
            bytes.toHexString() shouldBe "bf8a3d0a0c0566697273740c0133"
            DER.decodeFromSource<WithThreeNullable>(Buffer().apply { write(bytes) }) shouldBe withThreeOtherNull
        }

        val withThreeOther = WithThreeNullable("first", 2, "3")

        Buffer().apply { DER.encodeToSink(withThreeOther, this) }.readByteArray().also { bytes ->
            bytes.toHexString() shouldBe "bf8a3d0d0c0566697273740201020c0133"
            DER.decodeFromSource<WithThreeNullable>(Buffer().apply { write(bytes) }) shouldBe withThreeOther
        }

        val without = Without
        val withOne = WithOne("")
        val withTwo = WithTwo("1", "2")
        val withTwoOther = WithTwoOther("1", 2)
        val withThree = WithThree("1", "3", "3")

        Buffer().apply { DER.encodeToSink(without, this) }.readByteArray().also { bytes ->
            bytes.toHexString() shouldBe "3000"
            DER.decodeFromSource<Without>(Buffer().apply { write(bytes) }) shouldBe without
        }

        shouldThrow<SerializationException> { DER.decodeFromSource<Without>(Buffer().apply { write("30020c00".hexToByteArray()) }) }

        Buffer().apply { DER.encodeToSink(withOne, this) }.readByteArray().also { bytes ->
            bytes.toHexString() shouldBe "bf8a39020c00"
            DER.decodeFromSource<WithOne>(Buffer().apply { write(bytes) }) shouldBe withOne
        }

        Buffer().apply { DER.encodeToSink(withTwo, this) }.readByteArray().also { bytes ->
            bytes.toHexString() shouldBe "bf8a3a060c01310c0132"
            DER.decodeFromSource<WithTwo>(Buffer().apply { write(bytes) }) shouldBe withTwo
        }

        Buffer().apply { DER.encodeToSink(withTwoOther, this) }.readByteArray()
            .also { bytes ->
                bytes.toHexString() shouldBe "bf8a3b060c0131020102"
                DER.decodeFromSource<WithTwoOther>(Buffer().apply { write(bytes) }) shouldBe withTwoOther
            }

        Buffer().apply { DER.encodeToSink(withThree, this) }.readByteArray().also { bytes ->
            bytes.toHexString() shouldBe "bf8a3c090c01310c01330c0133"
            DER.decodeFromSource<WithThree>(Buffer().apply { write(bytes) }) shouldBe withThree
        }
    }

    "Polymorphic" {
        val without = Without
        val withOne = WithOne("")
        val withTwo = WithTwo("1", "2")
        val withTwoOther = WithTwoOther("1", 2)
        val withThree = WithThree("1", "3", "3")


        //not registered
        shouldThrow<SerializationException> {
            DER.decodeFromSource<List<AnInterface>>(Buffer().apply { write(
                "30293000bf8a39020c00bf8a3a060c01310c0132bf8a3b060c0131020102bf8a3c090c01310c01330c0133".hexToByteArray()
            ) })
        }
        val der = DER {
            serializersModule = SerializersModule {
                polymorphicByTag(AnInterface::class) {
                    subtype<Without>()
                    subtype<WithOne>()
                    subtype<WithTwo>()
                    subtype<WithTwoOther>()
                    subtype<WithThree>()
                }
            }
        }

        val listOf = listOf(without, withOne, withTwo, withTwoOther, withThree)
        //Registered
        der.decodeFromSource<List<AnInterface>>(Buffer().apply { write(
            "30293000bf8a39020c00bf8a3a060c01310c0132bf8a3b060c0131020102bf8a3c090c01310c01330c0133".hexToByteArray()
        ) }) shouldBe listOf


        //junk
        shouldThrow<SerializationException> {
            der.decodeFromSource<List<AnInterface>>(Buffer().apply { write(
                "".hexToByteArray()
            ) })
        }

        der.decodeFromSource<List<AnInterface>>(Buffer().apply { write(
            Buffer().apply { der.encodeToSink(listOf, this) }.readByteArray()
                .also { println(it.toHexString()) }
        ) }) shouldBe listOf
    }

    "Choice polymorphism (sealed only)" {
        val intChoice = ChoiceContainer(ChoiceInt(7))
        val taggedStringChoice = ChoiceContainer(ChoiceTaggedString("foo"))

        DER.decodeFromSource<ChoiceContainer>(Buffer().apply { write(
            Buffer().apply { DER.encodeToSink(intChoice, this) }.readByteArray()
        ) }) shouldBe intChoice
        DER.decodeFromSource<ChoiceContainer>(Buffer().apply { write(
            Buffer().apply { DER.encodeToSink(taggedStringChoice, this) }.readByteArray()
        ) }) shouldBe taggedStringChoice

        val list = listOf<ChoiceInterface>(ChoiceInt(1), ChoiceTaggedString("bar"))
        DER.decodeFromSource<List<ChoiceInterface>>(Buffer().apply { write(
            Buffer().apply { DER.encodeToSink(list, this) }.readByteArray()
        ) }) shouldBe list
    }

    "Choice ambiguity is rejected at runtime" {
        val encoded = Buffer().apply { DER.encodeToSink(

            AmbiguousChoiceA("foo")
        , this) }.readByteArray()
        shouldThrow<SerializationException> {
            DER.decodeFromSource<AmbiguousChoice>(Buffer().apply { write(encoded) })
        }
    }
}

interface AnInterface

// @formatter:off
@Serializable object Without : AnInterface

@Asn1Tag(1337u)
@Serializable data class WithOne(val first: String) : AnInterface
@Asn1Tag(1338u)
@Serializable data class WithTwo(val first: String, val second: String) : AnInterface
@Asn1Tag(1339u)
@Serializable data class WithTwoOther(val first: String, val second: Int) : AnInterface
@Asn1Tag(1340u)
@Serializable data class WithThree(val first: String, val second: String, val third: String) : AnInterface
@Asn1Tag(1341u)
@Serializable data class WithThreeNullable(val first: String, val second: Int?, val third: String)
// @formatter:on

@Serializable
sealed interface ChoiceInterface

@Serializable
data class ChoiceContainer(val choice: ChoiceInterface)

@Serializable
data class ChoiceInt(val value: Int) : ChoiceInterface

@Serializable
@Asn1Tag(tagNumber = 1u)
data class ChoiceTaggedString(val value: String) : ChoiceInterface

@Serializable
sealed interface AmbiguousChoice

@Serializable
data class AmbiguousChoiceA(val value: String) : AmbiguousChoice

@Serializable
data class AmbiguousChoiceB(val value: String) : AmbiguousChoice
