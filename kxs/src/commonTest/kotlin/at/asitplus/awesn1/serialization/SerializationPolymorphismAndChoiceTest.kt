...
val SerializationTestPolymorphismAndChoice by testSuite(
    ...
    "Number of elements" {
        ...
        DER.encodeToByteArray(withTwoOther)
            .apply {
    ...
    "Choice polymorphism (sealed only)" {
        ...
        DER.decodeFromByteArray<ChoiceContainer>(
            DER.encodeToByteArray(intChoice)
        ) shouldBe intChoice
        DER.decodeFromByteArray<ChoiceContainer>(
            DER.encodeToByteArray(taggedStringChoice)
        ) shouldBe taggedStringChoice

        val list = listOf<ChoiceInterface>(ChoiceInt(1), ChoiceTaggedString("bar"))
        DER.decodeFromByteArray<List<ChoiceInterface>>(
            DER.encodeToByteArray(list)
        ) shouldBe list
    }

    "Choice ambiguity is rejected at runtime" {
        val encoded = DER.encodeToByteArray(
