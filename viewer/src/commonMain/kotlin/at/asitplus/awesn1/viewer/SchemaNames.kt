...
fun schemaMemberNames(bytes: ByteArray, root: Asn1Element): Map<String, String> {
    ...
    if (children.size >= 9 && children.all { it.tag == Asn1Element.Tag.INT })
        decode(Pkcs1RsaPrivateKeyInfo.serializer(), bytes)?.let { return Names(root).rsaPrivate("0") }
    if (children.size in 2..4 && children[0].tag == Asn1Element.Tag.INT && children[1].tag == Asn1Element.Tag.OCTET_STRING)
        decode(Sec1EcPrivateKeyInfo.serializer(), bytes)?.let { return Names(root).ecPrivate("0") }
    if (children.size == 2 && children[0].tag == Asn1Element.Tag.SEQUENCE && children[1].tag == Asn1Element.Tag.BIT_STRING)
        decode(SubjectPublicKeyInfo.serializer(), bytes)?.let { return Names(root).subjectPublicKeyInfo("0", "SubjectPublicKeyInfo") }
    if (children.size == 2 && children.all { it.tag == Asn1Element.Tag.INT })
        decode(Pkcs1RsaPublicKeyInfo.serializer(), bytes)?.let { return Names(root).rsaPublic("0") }
    return emptyMap()
}
...
private class Names(private val root: Asn1Element) {
    ...
    fun certificate(): Map<String, String> = apply {
        name("0", "Certificate")
        name("0.0", "tbsCertificate")
        val tbs = "0.0"
        var index = 0
        ...
        while (node(tbs).childCount > index) {
            val path = "$tbs.$index"
            when (node(path)?.tag?.tagValue) {
                1uL -> name(path, "issuerUniqueID")
                2uL -> name(path, "subjectUniqueID")
                3uL -> extensions(path)
            }
            index++
        }
        algorithm("0.1", "signatureAlgorithm")
        name("0.2", "signatureValue")
    }.result

    fun csr(): Map<String, String> = apply {
        name("0", "CertificationRequest")
        name("0.0", "certificationRequestInfo")
        name("0.0.0", "version")
        x500Name("0.0.1", "subject")
        subjectPublicKeyInfo("0.0.2", "subjectPKInfo")
        val attributes = "0.0.3"
    ...
    fun pkcs8(key: Pkcs8PrivateKeyInfo): Map<String, String> = apply {
        name("0", "PrivateKeyInfo")
        name("0.0", "version")
        algorithm("0.1", "privateKeyAlgorithm")
        name("0.2", "privateKey")
        if (node("0").childCount > 3) name("0.3", "attributes")
        when {
            runCatching { Pkcs1RsaPrivateKeyInfo.of(key) }.isSuccess -> rsaPrivate("0.2.0", "RSAPrivateKey")
            runCatching { Sec1EcPrivateKeyInfo.of(key) }.isSuccess -> ecPrivate("0.2.0", "ECPrivateKey")
        }
    }.result

    fun encryptedPrivateKey(): Map<String, String> = apply {
        name("0", "EncryptedPrivateKeyInfo")
        algorithm("0.0", "encryptionAlgorithm")
        name("0.1", "encryptedData")
    }.result

    fun rsaPrivate(path: String, rootName: String = "RSAPrivateKey"): Map<String, String> = apply {
        name(path, rootName)
        listOf("version", "modulus", "publicExponent", "privateExponent", "prime1", "prime2", "exponent1", "exponent2", "coefficient")
            .forEachIndexed { index, member -> name("$path.$index", member) }
        if (node(path).childCount > 9) {
            val others = "$path.9"
    ...
    fun ecPrivate(path: String, rootName: String = "ECPrivateKey"): Map<String, String> = apply {
        name(path, rootName)
        name("$path.0", "version")
        name("$path.1", "privateKey")
        for (index in 2 until node(path).childCount) {
    ...
    fun subjectPublicKeyInfo(path: String, rootName: String): Map<String, String> = apply {
        name(path, rootName)
        algorithm("$path.0", "algorithm")
        name("$path.1", "subjectPublicKey")
    }.result

    fun rsaPublic(path: String): Map<String, String> = apply {
        name(path, "RSAPublicKey")
        name("$path.0", "modulus")
        name("$path.1", "publicExponent")
    }.result

    private fun algorithm(path: String, member: String) {
        name(path, member)
        name("$path.0", "algorithm")
        if (node(path).childCount > 1) name("$path.1", "parameters")
    }

    private fun x500Name(path: String, member: String) {
        name(path, member)
        node(path).children.forEachIndexed { rdnIndex, rdn ->
            val rdnPath = "$path.$rdnIndex"
    ...
    private fun extensions(taggedPath: String) {
    ...
    private fun name(path: String, value: String) {
        if (node(path) != null) result[path] = value
    }

    private fun node(path: String): Asn1Element? = path.split('.').drop(1).fold(root as Asn1Element?) { current, part ->
        current.children.getOrNull(part.toInt())
    }
